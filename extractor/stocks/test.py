...
#1. get info breaks on crypto => check why
...
for key in dict_assets.keys():
    ...
    res_historical_data = hist_dr.get_historical_data(tickers,period="max")
    res_info_data = info_dr.get_info_data(tickers,period="max")
    #resource = AccountingDataDataSource().get_balance_sheet(ticker)
    res_analyst_price_targets = analysts_dr.get_analyst_price_targets(tickers)
