                ...
                if len(sub_res) > 0:
                    for sub_res_item in sub_res:
                        if sub_res_item is not None:
                            ...
                            sub_res_item = utl.add_audit_info(
                                dest=sub_res_item,
                                updated_at=updated_at,
                                updated_by=updated_by,
                            )
                            # nested child dics decoration
                            sub_res_item = utl.add_audit_info_nested_dictionaries(
                                name=name,
                                dict=sub_res_item,
                                updated_at=updated_at,
                                updated_by=updated_by,
                            ...
                            print(
                                f"get_news_data -> None, i.e. not supported for the ticker: {ticker}"
                            )
                    if len(res) == 0:
